...
3. Fill out the scheduling modal with:
   ...
   * **Customization** (FT & DRS only) - Party count and role composition (e.g., "6 parties 1t2h5d")
...
* **FT & DRS Runs** - Role-based signups with reserve system, automatic roster distribution
...
4. Bot posts the resource directly to the channel or privately
...
Use the dropdown menu to select runs to withdraw from..

NOTE: If you're blacklisted from FT signups, this command will inform you and show the reason.

---

=== /forked-preferences & /drs-preferences

Manage your personal preferences for FT & DRS raids including party friend lists and phantom job selections. These selections allow Raid Leads utilizing the CAFE Roster Manager to better place you where you want to go! 
...
* Maximum 5 party homies

**Phantom Jobs & DRS Roles:**

* Select your preferred phantom jobs/roles for either raid
* Bot considers these during automatic roster creation
* Helps raid leads optimize party composition
...
TIP: Setting party homies early increases your chances of being grouped together!
...
2. Modal appears with current period's prog point options
3. Select which prog points you'd like
4. Submit your vote
...
* Polls reset bi-weekly
* One vote per user per period
* Can update your vote before reset
* Results may influence raid leader prog focus
...
[source,discord]
----
/runaudit runtype:<ba|drs> timerange:<week1|week2|month1|month3>
----
...
* Change run time
* Update description
* Modify party configuration (FT/DRS only)
...
Context menu commands are accessed by right-clicking (desktop) or long-pressing (mobile) on messages or users.
